...
exclude = [ str(p.name) for p in repo_dir.glob('.*') ]
...
def parse_repo():
    ...
    for base, dirs, files in os.walk(services_dir):
            ...
            if file_name == "Dockerfile":
                ...
                for service_name, service in content['services'].items():
                    if 'labels' in service['build']:
                        info = {
                            'folder': f'{relbase}/' + os.path.dirname(service['build'].get('dockerfile','')),
                            'image': service['image']
                        }
        ...
        if docker_files and version_files and services_info:
            break
